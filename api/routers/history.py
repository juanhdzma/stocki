...
async def price_history(ticker: str, period: str = "1y"):
    ...
    if period not in _ALLOWED_PERIODS:
        raise HTTPException(status_code=400, detail=f"period must be one of {sorted(_ALLOWED_PERIODS)}")

    try:
        points = await asyncio.to_thread(fetch_price_history, ticker, period)
    except Exception as exc:
        raise HTTPException(status_code=502, detail=f"Failed to fetch price history for {ticker}: {exc}")
