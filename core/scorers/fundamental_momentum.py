...
def score(fundamentals: list[dict], snapshot: dict) -> dict:
    ...
    if len(ni_all) >= 2:
        ...
        if ni_older < 0 and ni_recent > 0:
            ni_s = min(28, ni_s + 4)
        elif ni_older < 0 < delta:
            ni_s = min(28, ni_s + 2)
    ...
    if len(fcf_all) >= 2:
        ...
        rev_scale = rev_chron[-1] if rev_chron else None
        if rev_scale and rev_scale != 0:
            sub["fcf_trajectory"] = round(
                clamp((delta / abs(rev_scale) / 0.05 + 1) / 2 * 10, 0, 10), 1
            )
        else:
    ...
    if (
        price
        and price > 0
        and est_now is not None
        and est_old is not None
        and not currency_mismatch(snapshot)
    ):
