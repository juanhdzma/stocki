        ...
        out.append(
            {
                ...
                "delta_own": t["delta_own"]
                if "delta_own" in t and t["delta_own"] is not None
                else _parse_pct(t.get("ΔOwn")),
                "value_usd": t["value_usd"]
                if "value_usd" in t and t["value_usd"] is not None
...
def _filter(txs: list[dict], market_cap: float | None) -> list[dict]:
    ...
    for tx in txs:
        ...
        if tt == "S+OE":
            ...
            has_s_after = (
                any(timedelta(0) <= s_dt - td <= timedelta(days=2) for s_dt in s_dates.get(nm, []))
                if td
                else False
            )
            if has_s_after:
...
def _schedule_discount(trades: list[dict]) -> float:
    ...
    if 5 <= m <= 35:
        if cv < 0.30:
            return 0.20  # highly scheduled → 80% discount
        if cv < 0.60:
            return 0.50  # partially scheduled → 50% discount
    return 1.0
...
def compute_insider_score(
    ...
    bear_power = sum(
        ...
        * discounts.get(tx["insider_name"], 1.0)
        ...
        week52_low,
        week52_high,
        _POST_EARNINGS_SELL_MULTIPLIER,
        discounts,
    )
